    ...
    if request.method == "POST":
        ...
        if not users.login(username, password):
            return render_template(
                "index.html",
                message="Väärä käyttäjätunnus tai salasana, yritä uudelleen!")
        return redirect("/")
    return render_template("index.html")
...
def register():
    ...
    if request.method == "POST":
        ...
        if not users.check_passwords(password1, password2):
            return render_template(
                "register.html", message="Salasanat eroavat")
        if not users.check_password(password1):
            return render_template(
                "register.html", message="Salasana on tyhjä")
...
def base():
    if users.user_id():
        if not users.require_role(2):
            return render_template(
                "error.html",
                message="Sinulla ei ole oikeuksia luoda kyselyitä",
                route="/")
...
def new():
    if users.user_id():
        ...
        if topic.capitalize() in quizzes.get_topics():
            return render_template(
                "base.html",
                message=f"Kysely '{topic}' on jo olemassa, koita keksiä uusi aihe!")
        ...
        return render_template(
            ...
            topic=topic,
            quiz_type=quiz_type,
            questions=nmr_of_questions,
            choices=nmr_of_choices)
    return render_template(
        "error.html",
        message="Et ole kirjautunut sisään",
        route="/")
...
def create():
    if users.user_id():
        ...
        if quiz_type == 1 and nmr_of_questions != len(
                [x for x in correct if x == "True"]):
            return render_template(
                "newquiz.html",
                quiz_type=quiz_type,
                questions=nmr_of_questions,
...
def answer():
    if users.user_id():
        ...
        choice_ids = [request.form[question]
                      for question in question_ids if question in request.form]
        errormessage = ""
        if not choice_ids:
    ...
    if users.user_id():
        if users.require_role(2):
            ...
            if request.method == "POST":
                ...
                return render_template(
                    "pick.html", list=all_quizzes, message=True)
        return render_template(
            "error.html",
            message="Sinulla ei ole oikeuksia muokata kyselyitä",
            route="/")
    return render_template(
        "error.html",
        message="Et ole kirjautunut sisään",
        ...
        if users.require_role(2):
            ...
            if request.method == "POST":
                ...
                for choice in newchoices:
                    if choice == "" or len(choice) > 50:
                        return render_template(
                            ...
                            message="Vaihtoehdossa tulee olla 1-50 merkkiä")
                if quiz_type == 1 and nmr_of_questions != len(
                        [x for x in newcorrect if x == "True"]):
                    return render_template(
                        "edit.html",
                        quiz_id=id,
                        quiz_type=quiz_type,
                ...
                for question in newquestions:
                    ...
                    for choice in range(int(nmr_of_choices)):
                        quizzes.edit_choice(
                            choice_ids[i], question_id, newchoices[i], newcorrect[i])
                        i += 1
        ...
        if users.require_role(2):
            ...
            if request.method == "POST":
                ...
                if "quiz" in request.form:
                    ...
                    return redirect("/")
                return render_template(
                    "remove.html", list=all_quizzes, message=True)
        return render_template(
            "error.html",
            message="Sinulla ei ole oikeuksia poistaa kyselyitä",
            route="/")
    return render_template(
        "error.html",
        message="Et ole kirjautunut sisään",
